...
- `src/scrapers/news-discovery-scraper.ts` — Web横断検索によるマイナー助成金の発掘
  ...
  という活動報告から助成金名をAI抽出）の2系統。URLの解決（`resolveOfficialSite`）では、
  検索結果がプレスリリース配信サイト（PR TIMES 等、`PRESS_RELEASE_SITES`）だったら
  記事内の公式リンクを辿り、無ければ配信サイト除外で再検索する（読み手を公式ページへ
  飛ばすため）。フォームサービス（tayori 等）は `NON_OFFICIAL` で除外。
- `src/scrapers/watch-site-scraper.ts` — 人間がダッシュボードから登録した
  **巡回サイト**（`watch_sites` テーブル。助成金まとめページ・財団のお知らせ一覧など）を
  毎週フェッチし、助成金らしいリンクを汎用ロジックで拾って🔎新着・発見に載せる。
  サイト固有の解析は持たず、内容確認はAI読み取りと人間の判定に任せる。
...
- `src/server.ts` — 依存ライブラリ不要の `http` ダッシュボード。`GET /`（操作パネル）、
  ...
  `POST /api/add-grant`（助成金の手動追加。`source='manual'`＋`関係あり`判定で登録し
  その場でAI読み取り→以後は毎週の募集開始チェック対象）、
  `GET/POST /api/watch-sites`・`POST /api/watch-sites/delete`（巡回サイトの一覧/追加/削除）。
...
- **上書きしない**：スクレイパーが既に良い値を持つ項目（`要確認`/`不明` 以外）は
  AI の抽出結果で上書きしません。
- **締切による昇格**（`promoteIfDeadlineOpen`）：読み取れた締切が未来の日付（1年以内）なら
  `不明`→`募集中` に昇格させます（⚪要確認に居座らせない）。`募集前` の昇格は
  📎手動URL登録時のみ（前年から引き継いだ年なし締切での誤昇格防止）。
