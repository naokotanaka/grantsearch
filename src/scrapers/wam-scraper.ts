...
export class WamScraper extends BaseScraper {
  ...
  async search(): Promise<Grant[]> {
    ...
    try {
      ...
      $('a').each((_, elem) => {
        ...
        if (!/令和\d+年度.*助成.*(募集|お知らせ)|20\d{2}年度.*助成.*募集/.test(title)) return;

        const type = /モデル/.test(title) ? 'モデル事業'
          : /補正/.test(title) ? '補正予算事業'
          : '通常助成事業';
      ...
      for (const [type, info] of latestByType) {
        try {
          const grant = await this.buildFromDetailPage(type, info.title, info.url);
          if (grant) grants.push(grant);
        } catch (error) {
          console.error(`[WAM] 詳細ページの取得に失敗 (${info.url}):`, error instanceof Error ? error.message : error);
        }
      }

      if (grants.length === 0) {
        console.error('[WAM] 募集情報を抽出できませんでした（ページ構成が変わった可能性があります）');
      }
    } catch (error) {
  ...
  private async buildFromDetailPage(type: string, title: string, url: string): Promise<Grant | null> {
    const $ = await this.fetchPage(url);
    const text = this.cleanText($('body').text());

    // 「応募締切 令和N年M月D日」を探す
    const deadlineMatch = text.match(/応募締切[^令0-9]{0,10}((?:令和\d+年)?\d{1,2}月\d{1,2}日[^ ]{0,15})/);
    const deadlineText = deadlineMatch ? this.cleanText(deadlineMatch[1]) : '';
    const deadlineDate = deadlineText ? this.parseJapaneseDate(deadlineText) : null;

    const now = new Date();
    const targetProjects = '子ども食堂・居場所づくり・生活困窮者支援など、社会福祉振興のためのNPO等の事業';
    ...
    const monthLabel = deadlineDate ? `例年${deadlineDate.getMonth() + 1}月頃締切` : '例年12月〜1月頃';
    return this.createGrant({
