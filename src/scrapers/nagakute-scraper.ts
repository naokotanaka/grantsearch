...
export class NagakuteScraper extends BaseScraper {
  ...
  private isRelevantGrant(name: string): boolean {
    ...
    if (NagakuteScraper.NOISE_PATTERNS.some((w) => name.includes(w)))
      return false;
    const hasField = NagakuteScraper.RELEVANT_KEYWORDS.some((kw) =>
      name.includes(kw),
    );
    const looksLikeGrant = NagakuteScraper.GRANT_TOKENS.some((t) =>
      name.includes(t),
    );
